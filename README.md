...
2. Note the services are enabled and started automatically after installation. You can manage them with systemd, e.g. to check status:
    
    ```
    systemctl status storm-nimbus
    systemctl status storm-ui
    systemctl status storm-logviewer
    ```
